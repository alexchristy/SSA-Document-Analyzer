...
class TestTableToFlights(unittest.TestCase):
    ...
    def test_al_udeid_1_72hr(self):
        ...
        # Convert tables to flights
        table1_flights = convert_72hr_table_to_flights(table1, origin_terminal)
        table2_flights = convert_72hr_table_to_flights(table2, origin_terminal)
        table3_flights = convert_72hr_table_to_flights(table3, origin_terminal)

        # Check that the flights are the same
        self.assertEqual(len(table1_flights), len(table1_flights))
        self.assertEqual(len(table2_flights), len(table2_flights))
        self.assertEqual(len(table3_flights), len(table3_flights))

        # Check that the converted flights are the same as the known good flights:
        # Table 1
        for i, flight in enumerate(table1_flights):
            self.assertEqual(flight, table1_flights[i])

        # Table 2
        for i, flight in enumerate(table2_flights):
            self.assertEqual(flight, table2_flights[i])

        # Table 3
        for i, flight in enumerate(table3_flights):
    ...
    def test_andersen_1_72hr(self):
        ...
        table3 = Table.load_state("tests/table-objects/andersen_1_72hr_table-4.pkl")
